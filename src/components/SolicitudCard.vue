<template>
  ...
  >
    ...
    <div v-if="solicitud.estado === 'pendiente' || !solicitud.estado" class="flex gap-3 pt-4 border-t">
      <button @click="$emit('accept', solicitud.id)" class="btn-accept flex-1">Aceptar</button>
      <button @click="$emit('reject', solicitud.id)" class="btn-reject flex-1">Rechazar</button>
    </div>
...
.badge-rejected { 
    ...
    color: #ffffff; }
.msg-pending, .msg-accepted, .msg-rejected { 
